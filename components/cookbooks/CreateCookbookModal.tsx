/**
 * CreateCookbookModal Component
 *
 * Modal for creating a new cookbook.
 * Includes name, description fields and cover image picker.
 */
...
interface CreateCookbookModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess?: (cookbookId: string) => void;
}
...
export function CreateCookbookModal({
  ...
  const createCookbook = useMutation(api.cookbooks.createCookbook);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Failed to create cookbook");
    } finally {
